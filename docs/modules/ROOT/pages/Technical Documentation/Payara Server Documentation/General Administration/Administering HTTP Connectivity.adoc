...
| Determines whether to add the `HttpOnly` flag to the `JSESSIONID` cookie. In practice, this will disallow JavaScript access to the cookie in the browser.
...
| *cacheTTL*                  | Specifies the time to live (TTL) in milliseconds of the virtual server cache for serving files. | 5000
|===
...
* `true` — The `Secure` attribute is included.
* `false` — The `Secure` attribute is not included.
* `dynamic` — The `Secure` attribute setting is determined by whether the HTTP connection is secure. This is the default value.
