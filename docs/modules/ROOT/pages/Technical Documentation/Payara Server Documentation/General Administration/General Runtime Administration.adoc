...
+
When disabled, this will be once all `InstanceLifecycleListener`s and services have processed the `server_startup` event, but before post-boot configuration is applied and previously deployed applications are loaded.
+
