...
Payara Server has a Maven plugin which can start and stop Payara Server and deploy the Jakarta EE application.

The plugin requires JDK 8 or higher and the latest version is currently `1.0.0-Alpha1`
...
*Aim*: The dev goal is designed to streamline development by enabling `exploded`, `autoDeploy`, `liveReload`, `keepState`, and `trimLog` to true, which facilitating auto compilation and deployment on saving files resulting in rapid compilation, and deployment for an efficient development workflow.
