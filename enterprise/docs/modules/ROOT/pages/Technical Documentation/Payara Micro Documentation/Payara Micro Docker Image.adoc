...
This dockerfile makes use of your own Payara Micro installation and will construct the checkpoint as part of the build.
Starting the image will restore the application from the checkpoint.
...
ENV CHECKPOINT_DIR=/opt/payara/checkpoint
WORKDIR /opt/payara
...
RUN java \
    ...
    -XX:CRaCCheckpointTo=/opt/payara/checkpoint \
    -XX:CRaCEngine=warp \
    -XX:CRaCEngineOptions=keep_running=false \
    -Dfish.payara.crac.checkpointAfterDeployment=true \
    -jar payara-micro.jar \
    --deploy $DEPLOY_DIR/myapplication.war \
    --rootdir /opt/payara/micro \
    --warmup || true

ENTRYPOINT ["java", "-XX:CRaCEngine=warp", "-XX:CRaCRestoreFrom=/opt/payara/checkpoint", "-jar", "payara-micro.jar"]
----
