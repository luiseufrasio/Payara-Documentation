...
Upgrading Payara requires to stop the domain and the SSH nodes. This features enables to upgrade the SSH nodes individually, to avoid having any downtime. This is only to upgrade between 2 versions of Payara 5 or 6, not from 5 to 6.
...
[source, shell]
----
asadmin> upgrade-server --username example-user --distribution web --version 5.84.0 --nodes localhost-domain
----
...
[source, shell]
----
asadmin> upgrade-server --username example-user --distribution web --version 5.84.0 --nodes node1
----
